...
Ce programme correspond au noeud Node4 connectée en i2c avec un capteur SHT31.
...
void screendisplay(void){
  factory_display.drawString(0, 10, "Temperature= ");
  factory_display.drawString(50, 10, String(temperature));
  factory_display.drawString(0, 30, "Humidity= ");
  factory_display.drawString(50, 30, String(humidity));
  ...
  if (operationDone) {
    ...
    if (transmitFlag) {
      ...
      if (gateway[1] == Node4) {
        ...
        float temperature=round2(sht31.readTemperature());
        float humidity=round2(sht31.readHumidity());
