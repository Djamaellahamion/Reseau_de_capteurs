...
Ce programme correspond au noeud Node2 et envoie le String "Node2" à la passerelle lorsqu'il reçoit un message, de la part de la passerelle, contenant son adresse.
*/
...
void loop() {
  ...
  if(operationDone) {
    ...
    if(transmitFlag) {
      ...
      // send another one
      //test permettant de vérifier que la gateway s'adresse au Node2
      if (gateway[1]==Node2){
      Serial.print(F("[SX1276] Sending another packet ... "));
      //envoie de la chaîne de caractère "Node2"
      transmissionState = radio.startTransmit("Node2");
      transmitFlag = true;
        if (state == RADIOLIB_ERR_NONE) {
