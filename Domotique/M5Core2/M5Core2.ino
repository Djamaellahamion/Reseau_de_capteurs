...
#include <M5Core2.h>
uint8_t Font =2 ; // taille police LCD
...
void setup() {
  ...
  //M5.Lcd.setTextSize(Font);    //Set the Text size to 4

  M5.Lcd.setTextDatum(TC_DATUM);  //Set text alignment to center-up alignment.
  Serial.begin(115200);
...
void loop() {
  ...
  if (client.publish(Post.c_str(), JSONmessageBuffer) == true) {
    ...
    M5.Lcd.drawString("Success sending message", 160, 200,Font);
  } else {
    Serial.println("Error sending message");
    M5.Lcd.drawString("Error sending message", 160, 200,Font);
  ...
  String sensor = "sensor: BME680";
  String temp = String("Temp: ")+(round2(bme.temperature))+String(" °C");
  String humid = String("Hum: ") + (round2(bme.humidity)) + String(" %RH");
  String pres = String("Pres: ") + (round2(bme.pressure / 100.0)) + " hPa";
  String voc = String("VOC: ") + round2(bme.gas_resistance / 1000.0) ;
  // M5.Lcd.setCursor(0, 20);
  // M5.Lcd.println(salle);
  ...
  M5.Lcd.drawString(salle, 160, 20,Font);
  M5.Lcd.drawString(sensor, 160, 50,Font);
  M5.Lcd.drawString(temp, 160, 80,Font);
  M5.Lcd.drawString(humid, 160, 110,Font);
  M5.Lcd.drawString(pres, 160, 140,Font);
  M5.Lcd.drawString(voc, 160, 170,Font);
